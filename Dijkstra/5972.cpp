...
using namespace std;
vector<pair<int, int> > map[50001]; // map[a].push_back({b, c}) -> a도시에서 b도시까지의 거리는 c
...
void dijkstra(int start){
    ...
    while(!pq.empty()){
        ...
        for(int i=0; i<map[now].size(); i++){ // 현재 도시에서 다른 도시들로 향하는 경로의 개수만큼 순회
            ...
            int nextpos = map[now][i].first;
            if(viacost < dist[nextpos]){
                dist[nextpos] = viacost; // 최솟값 갱신
                pq.push(make_pair(-viacost, nextpos)); // 우선순위 큐에 삽입
            }
        }
    }
}
...
int main(){
    ...
    while(M--){
        ...
        map[e].push_back(make_pair(s, val));
    } // 입력

    dijkstra(1); // 시작 도시에서 다익스트라 알고리즘 개시
    cout << dist[N] << endl;
    return 0;
}